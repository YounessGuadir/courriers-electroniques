import React from 'react'

export default function Note_fount() {
  return (
    <div>

<div class='flex items-center justify-center min-h-screen bg-gradient-to-br'>
    <div class='w-full max-w-lg px-10 py-8 mx-auto  rounded-lg shadow-xl'>
        <div class='max-w-md mx-auto space-y-6'>
            
                <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
                    <div class="mx-auto max-w-screen-sm text-center">
                        <h1 class="mb-4 text-7xl tracking-tight font-extrabold lg:text-9xl text-primary-600 dark:text-primary-500">500</h1>
                        <p class="mb-4 text-3xl tracking-tight font-bold md:text-4xl">Internal Server Error.</p>
                        <p class="mb-4 text-lg font-light  ">We are already working to solve the problem. </p>
                    </div>   
                </div>

        </div>
    </div>
</div>
    </div>
  )
}
